...
const { getPresignedUrl } = require('../r2')
...
router.get('/clients', async (_req, res) => {
  ...
  // Collect all portalContacts from all clients — these are the actual businesses
  const businesses = []
  for (const client of clients) {
    const contacts = client.portalContacts || []
    for (const contact of contacts) {
      let logoUrl = ''
      if (contact.logoKey) {
        try { logoUrl = await getPresignedUrl(contact.logoKey) } catch {}
      }
      businesses.push({
        id: contact.id,
        businessId: contact.businessId || contact.id,
        refClientId: contact.refClientId || '',
        parentClientId: client.id,
        adminName: adminMap[client.adminId] || 'Unknown',
